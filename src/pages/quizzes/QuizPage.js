    ...
    <Container fluid className="h-100">
      <Row className="h-vh">
        <Col className="black pb-3 text-white pt-5 " xs={12} sm={12} md={12} lg={3} xl={3}>
          <Col>
            <Link link to={"/courses/"+props.course} className="text-white float-right"><Icon icon="angle-double-left" /> Back</Link>
            <div className="h6">{props.course.title}</div>
          </Col>
        </Col>
        <Col className="h-vh">
          <WizardProvider>
            <Row className="h-100">
              <Col className="h-100 border-right pt-5" xs={8} sm={8} md={8} lg={6} xl={6}>
                <WizardDisplay steps={[(<div>This is the main Body component</div>), (<div>This is the second wizard components</div>)]} />
              </Col>
                ...
                <WizardContext.Consumer>
                  {Wizard=>(
                    <div>
                      ...
                      <div className="text-center h5"><strong>{props.quiz.title}</strong></div>
