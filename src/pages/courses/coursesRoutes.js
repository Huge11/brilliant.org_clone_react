...
function CoursesRoutes(){
  ...
  const createRoutes = courses => {
    return courses.map(course => <Route path={path+"/"+course.name} key={course.name} render={props => <CoursePage {...course} {...props} />} /> )
  }
  ...
  return(
    <Switch>
      ...
      <Route path="*" render={props => <CoursePage {...props} {...tempCoursesData} /> } />
    </Switch>
  )
}
